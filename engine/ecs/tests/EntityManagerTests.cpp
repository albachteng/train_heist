...
TEST_F(EntityManagerTest, ValidateHandle) {
  ...
  EXPECT_FALSE(manager.isValid(
      handle)); // Handle should be invalid after entity destruction
}
...
TEST_F(EntityManagerTest, GenerationIsolationComponentMask) {
  ...
  uint64_t componentBits =
      (1ULL << 0) | (1ULL << 5) | (1ULL << 12); // Position, Health, Weapon
  ...
  EXPECT_EQ(entity2.componentMask, 0ULL)
      << "New generation should start with empty component mask";
  EXPECT_FALSE(entity2.hasComponent(1ULL << 0))
      << "New generation should not have Position component";
  EXPECT_FALSE(entity2.hasComponent(1ULL << 5))
      << "New generation should not have Health component";
  EXPECT_FALSE(entity2.hasComponent(1ULL << 12))
      << "New generation should not have Weapon component";
  EXPECT_FALSE(entity2.hasComponents(componentBits))
      << "New generation should not have any old components";

  // Verify old entity handle is invalid after destruction
  EntityHandle oldHandle(entity1.id, entity1.generation);
  EXPECT_FALSE(manager.isValid(oldHandle))
      << "Old generation handle should be invalid";

  // Verify new entity handle is valid
  EntityHandle newHandle = manager.createHandle(entity2);
  EXPECT_TRUE(manager.isValid(newHandle))
      << "New generation handle should be valid";
}

// Test mark dead and reuse approach - dead entities remain in storage but are
// invalid
...
TEST_F(EntityManagerTest, ReuseDeadEntitySlots) {
  ...
  // Create several entities with different IDs, then destroy some to create
  // reuse pool
  ...
  std::vector<EntityID> destroyedIds = {entities[1].id, entities[2].id,
                                        entities[4].id};
  ...
  EXPECT_EQ(manager.getDeadEntityCount(),
            3); // 3 dead entity IDs available for reuse
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    EXPECT_TRUE(std::find(destroyedIds.begin(), destroyedIds.end(),
                          entity.id) != destroyedIds.end());
  ...
  EXPECT_EQ(manager.getDeadEntityCount(),
            1); // 1 dead entity ID still available for reuse
...
TEST_F(EntityManagerTest, EntityLookupReturnsCurrentGeneration) {
  ...
  Entity *retrieved1 =
      manager.getEntityByID(reusedId); // New method: get current entity by ID
...
TEST_F(EntityManagerTest, DeadEntityIterationPattern) {
  ...
  std::vector<const Entity *> allEntities =
      manager.getAllEntitiesForIteration(); // New method
...
TEST_F(EntityManagerTest, EntityStorageGrowthBehavior) {
  ...
  EXPECT_EQ(manager.getTotalEntityCount(),
            numEntities); // Storage doesn't shrink
...
// Test that generation increment doesn't accidentally invalidate unrelated
// entities
TEST_F(EntityManagerTest,
       GenerationIncrementDoesNotInvalidateUnrelatedEntities) {
  EntityManager manager;
  ...
  // Create new entity - should reuse entity2's ID but with incremented
  // generation
  Entity newEntity = manager.createEntity();
  EXPECT_EQ(newEntity.id, id2);       // Should reuse ID2
  EXPECT_EQ(newEntity.generation, 1); // Should have incremented generation

  // CRITICAL TEST: entity1 and entity3 should still be valid with original
  // generations
  EXPECT_TRUE(manager.isValid(entity1));
  EXPECT_TRUE(manager.isValid(entity3));
